...
class G4PhantomParameterisation : public G4VPVParameterisation
{
  ...
  // Dummy declarations ...
    ...
    inline void SetMaterials(boost::python::list &x) {
        std::vector<G4Material*> tmp;
        int len = boost::python::len(x);
        for(int i=0; i<len; i++) {
            tmp.push_back(boost::python::extract<G4Material*>(x[i]));
        }
        fMaterials = tmp;
    }

    inline void SetMaterialIndices( size_t* matInd );
    inline void SetMaterialIndices(boost::python::list &x) {
        int len = boost::python::len(x);
        size_t *tmp = new size_t[len];
        for(int i=0; i<len; i++) {
            tmp[i] = boost::python::extract<int>(x[i]);
        }
        fMaterialIndices = tmp;
    }
